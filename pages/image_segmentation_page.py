...
def render_image_segmentation_page():
    ...
    with tab1:
        ...
        with sub_tab1:
            threshold_val = st.slider("Value", 0, 255, value=st.session_state.seg_threshold_value, key="seg_threshold_val_temp")
        ...
        with sub_tab2:
            adaptive_method = st.selectbox("Method", SEG_ADAPTIVE_METHODS, index=0 if st.session_state.seg_adaptive_method == "mean" else 1, key="seg_adaptive_method_temp")
            if adaptive_method != st.session_state.seg_adaptive_method:
                st.session_state.seg_adaptive_method = adaptive_method
                st.rerun()
            block_size = st.slider("Block", 3, 31, step=2, value=st.session_state.seg_adaptive_block, key="seg_adaptive_block_temp")
            if block_size != st.session_state.seg_adaptive_block:
                st.session_state.seg_adaptive_block = block_size
                st.rerun()
            c_val = st.slider("C", 0, 20, value=st.session_state.seg_adaptive_c, key="seg_adaptive_c_temp")
    ...
    # ==================== TAB 2: EDGE-BASED ====================
    with tab2:
        edge_method = st.selectbox("Method", SEG_EDGE_METHODS, index=SEG_EDGE_METHODS.index(st.session_state.seg_edge_method), key="seg_edge_method_temp")
        ...
        if st.session_state.seg_edge_method == "Canny":
            ...
            if kernel_val != st.session_state.seg_edge_kernel:
                ...
                st.rerun()
            sobel_dir = st.selectbox("Dir", ["both", "x", "y"], index=["both", "x", "y"].index(st.session_state.seg_sobel_direction), key="seg_sobel_dir_temp")
        ...
        edge_mode_map = {"Canny": "edge_canny", "Sobel": "edge_sobel", "Laplacian": "edge_laplacian"}
    ...
    # ==================== TAB 3: REGION-BASED ====================
    with tab3:
        region_method = st.selectbox("Method", SEG_REGION_METHODS, index=SEG_REGION_METHODS.index(st.session_state.seg_region_method), key="seg_region_method_temp")
        ...
        if st.session_state.seg_region_method == "Region Growing":
            threshold_val = st.slider("Threshold", 0, 50, value=st.session_state.seg_region_threshold, key="seg_region_threshold_temp")
            ...
            k_val = st.slider("K", 2, 8, value=st.session_state.seg_kmeans_k, key="seg_kmeans_k_temp")
            if k_val != st.session_state.seg_kmeans_k:
                st.session_state.seg_kmeans_k = k_val
                st.rerun()
        elif st.session_state.seg_region_method == "Contour":
            contour_mode = st.selectbox("Mode", ["external", "all"], index=0 if st.session_state.seg_contour_mode == "external" else 1, key="seg_contour_mode_temp")
            if contour_mode != st.session_state.seg_contour_mode:
                st.session_state.seg_contour_mode = contour_mode
                st.rerun()
        
        region_mode_map = {"Region Growing": "region_growing", "K-Means": "kmeans", "Contour": "contour", "Watershed": "watershed"}
